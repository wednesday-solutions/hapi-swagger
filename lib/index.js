...
exports.plugin = {
  ...
  register: (server, options) => {
    ...
    if (settings.cache && !server.methods.getSwaggerJSON) {
      // set default
      settings.cache.segment = 'hapi-swaggerui';
      ...
      const options = {
        ...
        generateKey: (settings, request) => 'hapi-swaggerui-' + request.path
      };
      ...
      },
      {
        ...
        handler: {
          file: function(request) {
            return join(__dirname, `../public/${request.params.filename}`);
          }
        },
        options: {
          auth: false
